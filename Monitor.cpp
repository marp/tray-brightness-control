...
Monitor::Monitor(HMONITOR nonPhysicalMonitor)
{
	...
	nameW = monitorPointer.szPhysicalMonitorDescription;
...
void Monitor::printCapabilities()
{
	...
	OutputDebugString(L"	Brightness: ");
	OutputDebugString(supportsBrightness()? L"Yes": L"No");
	OutputDebugString(L"\n");
...
bool Monitor::initialiseMonitor(HMONITOR nonPhysicalMonitor)
{
	...
	else
	{
		...
		OutputDebugString(monitorInfo.szDevice);
	}
...
INT8 Monitor::GetCurrentAverageBrightnessOfAll() {  
   // Check if the monitor list is not empty  
   if (monitors.empty()) {  
       OutputDebugString(L"No monitors found.");  
       return -1;  
   }  

   DWORD totalBrightness = 0;  
   size_t count = 0;  

   // Iterate through monitors and calculate total brightness  
   for (Monitor* monitor : monitors) {  
       if (monitor->supportsBrightness()) {  
           ContinuousSetting* brightness = monitor->getBrightness();  
           if (brightness != nullptr) {  
               totalBrightness += brightness->current;  
               ++count;  
               delete brightness; // Free memory  
           }  
       }  
   }  
   ...
   return static_cast<INT8>(totalBrightness / count);  
}
...
BOOL Monitor::SetBrightnessForAll(unsigned long newValue) {
	BOOL isValid = true;
	for (Monitor* monitor : monitors) {
